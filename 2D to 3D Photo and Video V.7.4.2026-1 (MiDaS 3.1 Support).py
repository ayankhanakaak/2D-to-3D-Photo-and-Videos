# 2D to 3D Photo and Video V.7.4.2026-1 (MiDaS 3.1 Support)
'''
Made by: Ayan Khan
...
def fix_timm():
    ...
    # Step 1: Remove ALL timm from sys.modules
    to_delete = [m for m in sys.modules if m.startswith('timm')]
    for m in to_delete:
        del sys.modules[m]
    
    # Step 2: Find and delete timm installation directory
    try:
        import site
...
def precache_model(model_type: str):
    ...
    try:
        ...
        if "relative_position_index" in str(e) or "Unexpected key" in str(e):
            ...
            # Manually load with strict=False
...
def load_midas_model(model_type: str, device=None):
    """Load MiDaS model with BEiT compatibility fix"""
    try:
...
def get_midas_transform(model_type: str, midas_transforms):
    """Get appropriate transform for model type"""
...
try:
    ...
    if tuple(map(int, timm.__version__.split('.')[:2])) >= (1, 0):
        ...
        # Reinstall
        subprocess.check_call([
            sys.executable, "-m", "pip", "uninstall", "-y", "-q", "timm"
        ], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        
        subprocess.check_call([
            sys.executable, "-m", "pip", "install", "-q", "timm==0.6.13"
        ])
...
def main():
    try:
        ...
        try:
            ...
            if "Unexpected key" in str(e):
                log("Applying BEiT compatibility fix...")
                midas = torch.hub.load("intel-isl/MiDaS", model_type, pretrained=False, verbose=False)
                import glob
                checkpoint_dir = os.path.join(os.path.expanduser("~"), ".cache", "torch", "hub", "checkpoints")
                for fname in os.listdir(checkpoint_dir):
                    if "beit" in fname.lower() or "swin" in fname.lower() or "next_vit" in fname.lower():
                        state_dict = torch.load(os.path.join(checkpoint_dir, fname), map_location="cpu")
        ...
        # Get appropriate transform for model
        if "512" in model_type:
...
if use_cuda:
    ...
    if HAS_NVDEC:
        use_nvdec = 2#int(input("NVDEC (HW decode)?\n1. Yes\n2. No\n>>> ") or "2") == 1
    if HAS_NVENC:
    ...
    try:
        batch_size = 9#int(input("Batch size [4]: ") or "4")
    except:
        batch_size = 4
