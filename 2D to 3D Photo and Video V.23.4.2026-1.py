...
Fixes vs 7.4.2026-2:
  ...
  - FIX 2: single_gpu_process() now respects use_cuda_remap (uses CUDARemapper/CPURemapper)
  - FIX 3: Worker NVENCWriter now detects BrokenPipeError and falls back to StandardWriter
  - FIX 4: single_gpu_process() batch_size default corrected from 4 → 9

Performance optimisations vs 7.4.2026-2:
  - OPT 1: estimate_shift_crosscorr() has a GPU path via cv2.cuda.matchTemplate when available
  - OPT 2: apply_focus_shift() has a GPU path via cv2.cuda.warpAffine when available
  - OPT 3: Focus estimation is cached across frames — recomputed every `focus_cache_interval`
            frames (default = batch_size) instead of every single frame, since depth-generated
            stereo pairs have nearly constant shift within a batch
'''
...
def check_cuda_focus_support():
    """Check if cv2.cuda.matchTemplate and cv2.cuda.warpAffine are available"""
    try:
        if cv2.cuda.getCudaEnabledDeviceCount() > 0:
            # Test matchTemplate
            a = cv2.cuda_GpuMat()
...
# OPT 1 + OPT 2: GPU-accelerated focus estimation and warp

class CUDAFocusEstimator:
    """
    GPU-accelerated focus estimation (cv2.cuda.matchTemplate)
    and focus shift (cv2.cuda.warpAffine).
    """
    ...
    def estimate(self, left_gray: np.ndarray, right_gray: np.ndarray) -> int:
        """GPU matchTemplate on horizontal band"""
        h, w = left_gray.shape
        band_h = h // 3
        y1 = h // 2 - band_h // 2
        y2 = h // 2 + band_h // 2
    ...
    def apply_shift(self, left_view: np.ndarray, right_view: np.ndarray,
                    focus_px: int):
        """GPU warpAffine for focus shift"""
...
class CPUFocusEstimator:
    """CPU fallback for focus estimation and shift"""
...
def build_focus_estimator(use_cuda_focus: bool, w: int, h: int, max_disp: int = 300):
    """Factory: returns CUDAFocusEstimator if supported, else CPUFocusEstimator"""
    if use_cuda_focus:
        try:
...
# ---------- Anaglyph builder (takes pre-computed focus_px — no estimation here) ----------
def build_anaglyph(left_s: np.ndarray, right_s: np.ndarray,
                   anaglyph_mode: int) -> np.ndarray:
    """Assemble anaglyph channels from already-shifted left/right views"""
...
def create_anaglyph_frame(left_view: np.ndarray, right_view: np.ndarray,
                          anaglyph_mode: int,
                          focus_estimator,
                          cached_focus_px: list) -> np.ndarray:
    """
    Create one anaglyph frame using pre-cached focus_px.
    cached_focus_px is a 1-element list so callers can read the current value back.
    Pass cached_focus_px[0] = None to force re-estimation this frame.
    """
    if cached_focus_px[0] is None:
        ...
        cached_focus_px[0] = focus_estimator.estimate(gray_l, gray_r)

    left_s, right_s = focus_estimator.apply_shift(
        left_view, right_view, cached_focus_px[0])
    return build_anaglyph(left_s, right_s, anaglyph_mode)
...
class CPURemapper:
    ...
    def remap(self, frame_bgr, depth_norm):
        ...
        left_view  = cv2.remap(frame_bgr, map_x_left,  self.grid_y,
                               cv2.INTER_LINEAR, borderMode=cv2.BORDER_REPLICATE)
        right_view = cv2.remap(frame_bgr, map_x_right, self.grid_y,
                               cv2.INTER_LINEAR, borderMode=cv2.BORDER_REPLICATE)
        return left_view, right_view
...
def try_reencode_video(input_path: str, crf: int = 18) -> str:
    ...
    try:
        ...
        pbar = tqdm(total=duration, desc="FFmpeg Pre-encode", unit="sec",
                    mininterval=1, dynamic_ncols=True) if duration else None
        last_time = 0.0
            ...
            for fname in os.listdir(checkpoint_dir):
                if model_type.lower().replace("_", "") in fname.lower().replace("_", "") or \
                   any(k in fname for k in ["beit", "swin", "next_vit", "levit"] if k in model_type.lower()):
                    ...
                    print("✅ Loaded with strict=False (ignored extra keys)")
                    break
        else:
            raise
...
def main():
    try:
        ...
        cached_focus_px      = [None]   # 1-element list (mutable box)
        ...
        while frames_done < total_frames:
            ...
            for i in range(len(batch_frames)):
                ...
                if output_choice == 1:
                    # OPT 3: only re-estimate focus every focus_cache_interval frames
                    if frames_since_focus >= focus_cache_interval:
                        cached_focus_px[0] = None   # force re-estimation
                        frames_since_focus = 0
...
def single_gpu_process(model_type, video_path, out_path, max_shift, output_choice,
                       ...
                       focus_cache_interval=None, start_frame=0, end_frame=None):
    """
    focus_cache_interval: how many frames between focus re-estimations.
                          Defaults to batch_size (recompute once per batch).
    """
    ...
    try:
        while frames_written < total:
            ...
            for i, frame in enumerate(batch_frames):
                ...
                # FIX 2: use remapper object
                left, right = remapper.remap(frame, depth_norm)

                if output_choice == 1:
                    # OPT 3: invalidate cache every focus_cache_interval frames
                    if frames_since_focus >= focus_cache_interval:
                        cached_focus_px[0] = None
                        frames_since_focus = 0

                    # OPT 1: GPU or CPU estimation, only when cache is stale
                    if cached_focus_px[0] is None:
                        gray_l = cv2.cvtColor(left,  cv2.COLOR_BGR2GRAY)
                        gray_r = cv2.cvtColor(right, cv2.COLOR_BGR2GRAY)
                        cached_focus_px[0] = focus_est.estimate(gray_l, gray_r)

                    # OPT 2: GPU or CPU warpAffine
...
HAS_CUDA_FOCUS   = check_cuda_focus_support()  # OPT 1+2
...
while True:
    try:
        duration_type = 1 #int(input("Choose duration:\n 1. Full Video\n 2. Custom\n>>> ") or "1")
...
if duration_type == 1:
    start_frame = 0
    end_frame = None  # Will be dynamically set to max frames later
...
if detected_streams:
    ...
    extracted_streams = extract_streams(video_path, detected_streams, temp_stream_dir, temp_stream_prefix, start_frame / fps, total_frames / fps)
else:
...
if use_multi_gpu and num_gpus > 1:
    ...
    for i in range(num_gpus):
        ...
        config = {
            ...
            "use_cuda_focus": use_cuda_focus,              # OPT 1+2
            "focus_cache_interval": focus_cache_interval,  # OPT 3
        }
